...
_check_bash_hint() {
  ...
  # segment, peek past "VAR=val" env-var assignments AND past known
  # command-delegating runners (timeout, nice, time, env, ionice, chrt)
  # and their args, then check whether the actual command's basename
  # starts with the wrapper prefix. `sudo` is intentionally NOT in the
  # runner set -- it's a different security boundary, and agents who
  # really need sudo+wrapper should use the bypass sentinel.
  if [[ -n "$_WRAPPER_PREFIX" ]]; then
    local _norm="${_cmd//&&/$'\\n'}"
    _norm="${_norm//||/$'\\n'}"
    _norm="${_norm//[|;&]/$'\\n'}"
    local _seg _toks _exec _t _base _arg _i
    while IFS= read -r _seg; do
      ...
      while (( _i < ${#_toks[@]} )); do
        _t="${_toks[$_i]}"
        # VAR=val env-var prefix: skip.
        if [[ "$_t" =~ ^[A-Za-z_][A-Za-z0-9_]*= ]]; then
          ((_i++)); continue
        fi
        # Known command-delegating runner: peek past its args (flags,
        # numeric durations, env-var pairs) to find the real command.
        _base="${_t##*/}"
        case "$_base" in
