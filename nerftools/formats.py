...
_check_bash_hint() {
  ...
  # segment for any wrapper-prefixed basename -- intentionally lenient,
  # we don't try to parse the runner's argv precisely. `sudo` is NOT in
  # the runner set: it's a different security boundary, and agents who
  # really need sudo+wrapper should use the bypass sentinel.
  #
  # This detection is heuristic. Odd quoting, subshells, or runners not
  # in the allowlist can trip false positives -- the hint message tells
  # the agent that "I think this is wrong" is a valid bypass reason.
