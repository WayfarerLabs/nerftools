...
`--no-default` is passed. Each `.yaml` file declares tools within a package (one package per file by
convention, though multiple files can contribute to the same package via merge).
...
the same `package.name`. See the [manifest reference](../../docs/nerf-manifest.md) for the full
format.
