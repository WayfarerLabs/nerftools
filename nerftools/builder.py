...
def _build_script(tool_name: str, package_name: str, tool_spec: ToolSpec) -> str:
    ...
    parts.append("")
    parts.append('if [[ "${BASH_VERSINFO[0]:-0}" -lt 4 ]]; then')
    parts.append(f'  echo "error: {tool_name} requires bash 4+. Found bash ${{BASH_VERSION:-unknown}}" >&2')
    parts.append("  echo \"  hint: on macOS, install a newer bash via 'brew install bash'\" >&2")
    parts.append("  exit 1")
    parts.append("fi")
    parts.append("")
