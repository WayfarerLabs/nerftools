...
def generate(
    ...
    output: Annotated[
        ...
        typer.Option(
            "--outdir",
            help="Output directory. Defaults to ./out/<target>/ when omitted. "
            "If passed explicitly, only one --target may be selected.",
        ),
    ] = None,
    no_default: Annotated[bool, typer.Option("--no-default", help="Skip built-in manifests")] = False,
    ...
    if output is not None and len(target) > 1:
        typer.echo(
            "Error: --outdir may only be combined with a single --target. "
            "Omit --outdir to write each target to ./out/<target>/, or invoke generate once per target.",
            err=True,
        )
        raise typer.Exit(1)
    ...
    for t in target:
        out_dir = output if output is not None else Path("out") / t

        try:
            if t == "bin":
                from nerftools.builder import build_scripts

                written = build_scripts(
                    loaded, out_dir, keep_existing=keep_existing, force=force, prefix=prefix
                )
                for path in written:
                    typer.echo(f"  {path}")
